...
function getAmountRaised($teamName, $dateStart,$dateEnd){
	global $wpdb;
	$money = 0;
	$array = array();
	if($teamName == ""){
		//This is the global thermometer we get all the data from all the donators
		//We also need to make sure that the payment was received

		//We need to get all the amount of the inscriptions and also verify if the payment was received.



		$row = $wpdb->get_results($wpdb->prepare("SELECT wp_rg_lead_detail.value FROM wp_rg_lead_detail INNER JOIN wp_rg_lead ON wp_rg_lead_detail.lead_id = wp_rg_lead.id WHERE wp_rg_lead_detail.field_number = 6 AND ((wp_rg_lead_detail.form_id = 2 AND wp_rg_lead.payment_status = \"Approved\") OR (wp_rg_lead_detail.form_id = 7)) AND wp_rg_lead.date_created >= '%s' AND wp_rg_lead.date_created <= '%s'",array($dateStart,$dateEnd)));
		foreach($row as $row) {
			array_push($array, _toInt($row->value));
		}
		$row = $wpdb->get_results($wpdb->prepare("SELECT wp_rg_lead_detail.value FROM wp_rg_lead_detail INNER JOIN wp_rg_lead ON wp_rg_lead_detail.lead_id = wp_rg_lead.id WHERE wp_rg_lead_detail.field_number = 6 AND wp_rg_lead_detail.form_id = 10 AND wp_rg_lead.date_created >= '%s' AND wp_rg_lead.date_created <= '%s'",array($dateStart,$dateEnd)));
		foreach($row as $row) {
			array_push($array, _toInt($row->value));
		}

		//We add back the inscription cost via paypal
                $row = $wpdb->get_results($wpdb->prepare("SELECT wp_rg_lead_detail.value FROM wp_rg_lead_detail INNER JOIN wp_rg_lead ON wp_rg_lead_detail.lead_id = wp_rg_lead.id WHERE wp_rg_lead_detail.form_id = 8 AND wp_rg_lead.payment_status = 'Approved' AND CAST(wp_rg_lead_detail.field_number as decimal(5,1)) = 9.3 AND wp_rg_lead.date_created >= '%s' AND wp_rg_lead.date_created <= '%s'", array($dateStart,$dateEnd)));
                foreach($row as $row){
			array_push($array,$row->value *25);
		}

		$row = $wpdb->get_results($wpdb->prepare("SELECT wp_rg_lead_detail.value FROM wp_rg_lead_detail INNER JOIN wp_rg_lead ON wp_rg_lead_detail.lead_id = wp_rg_lead.id WHERE wp_rg_lead_detail.form_id = 11 AND wp_rg_lead.payment_status = 'Approved' AND CAST(wp_rg_lead_detail.field_number as decimal(5,1)) = 9.3 AND wp_rg_lead.date_created >= '%s' AND wp_rg_lead.date_created <= '%s'", array($dateStart,$dateEnd)));
                foreach($row as $row){
                        array_push($array,$row->value *25);
                }

	}else{
		//We have a specific teamName we can look for the data and get the amountRaised
		$team = $teamName;
		$row = $wpdb->get_results($wpdb->prepare("SELECT wp_rg_lead_detail.lead_id FROM wp_rg_lead_detail INNER JOIN wp_rg_lead ON wp_rg_lead_detail.lead_id = wp_rg_lead.id  WHERE wp_rg_lead_detail.field_number = 7 AND (wp_rg_lead_detail.form_id = 2 OR wp_rg_lead_detail.form_id = 7) AND wp_rg_lead_detail.value = '%s' AND wp_rg_lead.date_created >= '%s' AND wp_rg_lead.date_created <= '%s' ",array($team,$dateStart,$dateEnd)));
		foreach($row as $row) {
			$val = $wpdb->get_var($wpdb->prepare("SELECT wp_rg_lead_detail.value FROM wp_rg_lead_detail INNER JOIN wp_rg_lead ON wp_rg_lead_detail.lead_id = wp_rg_lead.id WHERE wp_rg_lead_detail.field_number = 6 AND wp_rg_lead_detail.lead_id =%d AND (wp_rg_lead_detail.form_id = 2 OR wp_rg_lead_detail.form_id = 7) AND (wp_rg_lead.payment_status = \"Approved\" OR wp_rg_lead_detail.form_id = 7)", array($row->lead_id)));
			array_push($array,_toInt($val));
		}
	//We add other the inscription amount of the team to the thermometer
			$row = $wpdb->get_results($wpdb->prepare("SELECT wp_rg_lead_detail.lead_id FROM wp_rg_lead_detail WHERE wp_rg_lead_detail.form_id = 8 AND wp_rg_lead_detail.value = '%s' ",array($team)));

		foreach($row as $row){
		$val = $wpdb->get_var($wpdb->prepare("SELECT wp_rg_lead_detail.value FROM wp_rg_lead_detail INNER JOIN wp_rg_lead ON wp_rg_lead_detail.lead_id = wp_rg_lead.id WHERE wp_rg_lead.id = %d AND wp_rg_lead.payment_status = 'Approved' AND CAST(wp_rg_lead_detail.field_number as decimal(5,1)) = 9.3 AND wp_rg_lead.date_created >= '%s' AND wp_rg_lead.date_created <= '%s'", array($row->lead_id,$dateStart,$dateEnd)));
		array_push($array,$val *25);
	}

		$row = $wpdb->get_results($wpdb->prepare("SELECT wp_rg_lead_detail.lead_id FROM wp_rg_lead_detail WHERE wp_rg_lead_detail.form_id = 11 AND wp_rg_lead_detail.value = '%s' ",array($team)));
		foreach($row as $row){
			$val = $wpdb->get_results($wpdb->prepare("SELECT wp_rg_lead_detail.value FROM wp_rg_lead_detail INNER JOIN wp_rg_lead ON wp_rg_lead_detail.lead_id = wp_rg_lead.id WHERE wp_rg_lead_detail.form_id = 11 AND wp_rg_lead.payment_status = 'Approved' AND CAST(wp_rg_lead_detail.field_number as decimal(5,1)) = 9.3 AND wp_rg_lead_detail.lead_id =%d AND wp_rg_lead.date_created >= '%s' AND wp_rg_lead.date_created <= '%s'", array($row->lead_id,$dateStart,$dateEnd)));
                foreach($val as $val){
                        array_push($array,$val->value *25);
                }
		}
	//We can add the amount from the admin form without any problem.
	$val = $wpdb->get_var($wpdb->prepare("SELECT wp_rg_lead_detail.value FROM wp_rg_lead_detail WHERE wp_rg_lead_detail.lead_id = ( SELECT wp_rg_lead_detail.lead_id FROM wp_rg_lead_detail INNER JOIN wp_rg_lead ON wp_rg_lead_detail.lead_id = wp_rg_lead.id WHERE wp_rg_lead_detail.form_id = 10 AND wp_rg_lead_detail.value = '%s' AND wp_rg_lead.date_created >= '%s' AND wp_rg_lead.date_created <= '%s' LIMIT 1) AND wp_rg_lead_detail.field_number = 6",array($team,$dateStart,$dateEnd)));
	array_push($array,_toInt($val));
	}
...
function thermometer_graphic($atts){
	...
	if($atts['raised'] == ""){
		...
		$atts['raised'] = getAmountRaised($atts['team'],$atts['datestart'],$atts['dateend']);
	}
